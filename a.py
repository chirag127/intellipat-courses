...
def getHealth(genes, health, first, last, d):
    totalHealth = 0
    for i in range(first, last + 1):
        if d.find(genes[i]) != -1:
            totalHealth += health[i] * d.count(genes[i])
    return totalHealth


def getHealthiest(genes, health, d):

    # the healthiest is the giving wrong answer for some reason
    healthiest = getHealth(genes, health, 0, len(genes) - 1, d)
    for i in range(len(d)):
        for j in range(i, len(d)):
            totalHealth = getHealth(genes, health, 0, len(genes) - 1, d[i:j + 1])
            if totalHealth > healthiest:
                healthiest = totalHealth
    return healthiest
    


def getUnhealthiest(genes, health, d):
    unhealthiest = getHealth(genes, health, 0, len(genes) - 1, d)
    for i in range(len(d)):
        for j in range(i, len(d)):
            totalHealth = getHealth(genes, health, 0, len(genes) - 1, d[i:j + 1])
            if totalHealth < unhealthiest:
                unhealthiest = totalHealth
    return unhealthiest
...
if __name__ == '__main__':
    ...
    print(getUnhealthiest(genes, health, d), getHealthiest(genes, health, d))
